...
class RNN(object):
    ...
    def __init__(self, hidden_size, sequence_length, vocab_size, learning_rate):
        ...
        self.b = np.ones((hidden_size, 1))

        self.V = np.array(np.random.randn(hidden_size, vocab_size))
        self.c = np.ones((vocab_size, 1))
    ...
    def __rnn_backward(self, dh, cache):
        ...
        for gradient in [dU, dW, db]:
            np.clip(gradient, -self.grad_clip, self.grad_clip)
